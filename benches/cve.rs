...
/// Mimics the pyarrowspace CVE-search benchmark corpus: CVE records
/// 2018-2025 embedded with an all-MiniLM-L6-v2 fine-tune (384 features),
/// searched with k=20 results.
const N: usize = 300_000;
const D: usize = 384;
const CLUSTERS: usize = 128;
const QUERY_POOL: usize = 20;
const K_RESULTS: usize = 20;
const ALPHA: f64 = 0.5;
const RANGE_EPS: f64 = 0.05;

/// Graph parameters from pyarrowspace/tests/test_8_CVE_db_sweep.py.
/// Default eps=1.0 leaves this data distribution with an empty feature
/// graph (nnz == dim, all lambdas degenerate).
...
fn bench_cve(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("cve");
    ...
    group.bench_with_input(
        BenchmarkId::new("lambda_aware", K_RESULTS),
        &K_RESULTS,
        |b, &k| {
            let mut i = 0usize;
            b.iter(|| {
    ...
    group.bench_with_input(
        BenchmarkId::new("hybrid", K_RESULTS),
        &K_RESULTS,
        |b, &k| {
            let mut i = 0usize;
            b.iter(|| {
    ...
    group.bench_with_input(
        BenchmarkId::new("linear_sorted", K_RESULTS),
        &K_RESULTS,
        |b, &k| {
            let mut i = 0usize;
            b.iter(|| {
...
criterion_group!(benches, bench_cve);
